...
describe('Escenario de prueba 47', function () {
    before(function () {
    ...
    it('Crear una página con datos traidos por api solo alfanumericos, salir del admin, y revisar que no está publicada', function () {
        ...
        cy.visit(`${url}/ghost/#/pages`)
        cy.get('li[class="gh-list-row gh-posts-list-item"]').contains(dataSelected.password).click()
        cy.get('textarea[class="gh-editor-title ember-text-area gh-input ember-view"]').should("have.value", dataSelected.password)
    })
})
